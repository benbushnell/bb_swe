...
class SongsList extends React.Component {
 constructor(props) {
    ...
    this.state  = {
      ...
      error: null,
      refreshing: false,
      base_url: "http://127.0.0.1:8000/musey/api"
    }
  }
  ...
  fetchDataFromApi = ()  => {
    const url = "http://127.0.0.1:8000/musey/api/songs";
    ...
    return (
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            <Text style={styles.menuText}>{item.artist_name}</Text>
            <Text style={styles.locText}>{item.avgRating}</Text>
            </View>}
